...
public class Activer {
    ...
    public static DoubleMatrix maxProb(DoubleMatrix X) {
    	DoubleMatrix prob = new DoubleMatrix(X.rows, X.columns);
    	for (int i = 0; i < X.rows; i++) {
            DoubleMatrix expMi = X.getRow(i);
            prob.putRow(i, expMi.div(expMi.sum()));
        }
        return prob;
    }
